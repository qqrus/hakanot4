  ...
  if (suggestions.length === 0 && nextCode !== previousCode) {
    suggestions.push(
      makeSuggestion(
        ...
        "Mock reviewer не нашел явных дефектов в новом диффе, но советует прогнать выполнение кода перед демонстрацией.",
        "Запустите код в sandbox и убедитесь, что stdout соответствует ожидаемому сценарию демо.",
      ),
    );
  }
